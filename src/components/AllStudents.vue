          ...
          <template v-slot:item="row">
            <tr>
              ...
              <td>{{ row.item.student.address }}</td>
               <td><v-btn text @click="editDetails(row.item.student)">Edit Details</v-btn></td>
              </tr>
          </template>
        </v-data-table>
      </v-card>
      <v-dialog v-model="studentDialog" transition="dialog-top-transition" max-width="600">
                 <template>
                       ...
                       <v-card>
                          ...
                          <v-row>
                            ...
                            <v-col cols="12" xs="3" sm="4" md="4" lg="4">
                                  <v-text-field
                                    ...
                                    :rules="[
                                      ...
                                      (LRN) =>(/^[0-9]+$/.test(LRN)==true && String(LRN).length <= 12) ||
                                        'LRN cannot be greater than 12 characters & must contain all numbers only!',
                                      (LRN) =>(/^[0-9]+$/.test(LRN)==true && String(LRN).length == 12  )|| 'LRN must be 12 characters & must contain all numbers only!',                  
                                    ]"
                                    :counter="12"
                                ...
                                <v-text-field
                                  ...
                                  :rules="[
                                    ...
                                    (v) => v <= 100 || 'Maximum average is 100',
                                     v=>/^[0-9]+$/.test(v)==true || 'Average must be a number!'
                                  ]"                         
                                  label="Average"
                                    type="number"
                                  required
                            ...
                            <v-col cols="12" xs="6" sm="4" md="4" lg="4">
                              <v-text-field
                                ...
                                name="age"
                                :rules="[(v) => !!v || 'Age is required',v=>/^[0-9]+$/.test(v)==true || 'Age must be a number!']"
                                label="Age"   
                                type="number"                            
                                required
                              ></v-text-field>
                          ...
                          <v-col cols="12" xs="6" sm="4" md="4" lg="4">
                            ...
                            ></v-text-field>
                             <v-text-field
                              ...
                              v-model="studentInfo.IP_community"
                              name="IP_Community"
                              :rules="[(IP_Community) => !!IP_Community || 'This field is required',(IP_Community)=>/^[a-z]/.test(IP_Community)==true && /[^a-zA-Z.ñÑ| ]/.test(IP_Community)==false || 'This special character is not allowed!']"
                              label="If yes, please specify"
                              required
                            ></v-text-field>
                          </v-col>
                        ...
                        <v-col cols="12" xs="6" sm="6" md="6" lg="6">
                          <v-text-field
                            ...
                            name="guardian"
                            :rules="[(guardian) => !!guardian || 'Guardian name is required',(guardian) =>(/^[a-z]/.test(guardian)==true && /[^a-zA-Z.ñÑ| ]/.test(guardian)==false)|| 'This special character is not allowed!']"
  ...
  created() {
    this.$axios.get("approvedEnrollment").then((response)=>{
      ...
      this.students=this.filteredStudents.filter(val=> {
         return val.student.created_at.substring(0,val.created_at.indexOf("-")) == this.year || val.student.created_at.includes(this.year);
     });
    })
    .catch((error) => {
        console.log(error);        
...
filterByYear(year){
  if (year == "All") {
        ...
        this.students=this.filteredStudents.filter(val=> {
          return val.student.created_at.substring(0,val.created_at.indexOf("-")) == year || val.student.created_at.includes(year);
        });
        this.year=year;
      }
},
...
editDetails(student){
  ...
  this.studentInfo.created_at=student.created_at.substring(0,student.created_at.indexOf("-")).concat("-",parseInt(student.created_at.substring(0,student.created_at.indexOf("-")))+1);
  ...
  computed: {
    csvData() {
      return this.students.map(item => ({
         SchoolYear:item.student.created_at.substring(0,item.student.created_at.indexOf("-")).concat("-",parseInt(item.student.created_at.substring(0,item.student.created_at.indexOf("-")))+1),
